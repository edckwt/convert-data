...
function convert_data_checkbox_categories($cats_id=''){
	...
	foreach($categories as $category){
		...
		foreach($categories_child as $sub){
			...
			$get_sub .= '<li>&nbsp;&nbsp;&nbsp;&nbsp;<label class="selectit"><input value="'.$sub_category_id.'" type="checkbox" name="post_category[]" id="in-category-'.$sub_category_id.'"'.$sub_checked.' /> '.esc_attr($get_sub_title->name).'</label></li>';
		}
		...
		$text .= '<li style="border-bottom:1px dotted #ccc; padding-bottom:7px;"><label class="selectit"><input value="'.$category_id.'" type="checkbox" name="post_category[]" id="in-category-'.$category_id.'"'.$checked.' /> '.esc_attr($get_title->name).'</label>';
		if($get_sub != ""){
			$text .= '<ul style="margin-top:7px;">'.$get_sub.'</ul>';
		}
		$text .= '</li>';
	}
	$text .= '</ul>';
